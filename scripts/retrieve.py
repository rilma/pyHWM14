...
Examples:
  ...
  %(prog)s --year 2023 --day 150 --time 12.0 --lon -105.0 --alt 300.0 --lat-range -90 90 30
    ...
    date_group.add_argument(
        "--datetime", type=str, help='Date and time in format "YYYY-MM-DD HH:MM:SS"'
    )
    date_group.add_argument("--year", type=int, help="Year (YYYY)")

    parser.add_argument("--day", type=int, help="Day of year (1-366)")
    parser.add_argument("--time", type=float, help="Universal time in decimal hours (0-24)")
    ...
    alt_group.add_argument(
        "--alt", "--altitude", type=float, dest="altitude", help="Altitude in kilometers"
    )
    alt_group.add_argument(
        "--alt-range",
        nargs=3,
    ...
    # Other parameters
    parser.add_argument("--ap", type=int, default=10, help="Geomagnetic ap index (default: 10)")
    ...
    try:
        ...
        if args.lat_range:
            ...
            results = retrieve_single_point(
                year, day, time, args.latitude, args.longitude, args.altitude, args.ap, args.verbose
            )
